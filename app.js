...
app.post('/interactions', verifyKeyMiddleware(process.env.PUBLIC_KEY), async function (req, res) {
  ...
  if (type === InteractionType.MESSAGE_COMPONENT) {
    ...
    if (custom_id === 'cleaning_done') {
      // Get today's date in Eastern Time
      const todayEastern = new Date().toLocaleDateString('sv-SE', { timeZone: 'America/New_York' });
      const username = member?.user?.username || user?.username || 'Someone';
      const result = markComplete(todayEastern, username);
